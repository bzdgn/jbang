...
Om deze inhoud in andere talen lezen: :: link:readme.adoc[English], link:readme.tr.adoc[T¸rkÁe], link:readme.nl.adoc[Nederlands] 

== Intro

Wil je Java leren, ontdekken of gebruiken zonder setup?

Gebruik je python, groovy, kotlin of soortgelijke talen om scripts te schrijven hoewel je van Java houdt?

Heb je ooit Java 10+ support geprobeerd te gebruiken om `.java` bestanden rechtstreeks in je shell uit te voeren, maar vond je het een beetje te omslachtig?

Dan moet je proberen `jbang` waarmee je dit kunt doen:
...
Instant cli-app gegenereerd built met Java. En picocli is opgehaald als een dependency omdat het voor de compilatie en uitvoering is nodig.
