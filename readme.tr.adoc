...
Bu içeriği diğer dillerde okumak için: :: link:readme.adoc[English], link:readme.tr.adoc[Türkçe], link:readme.nl.adoc[Nederlands] 

== Intro

Java'yı kurulum olmadan öğrenmek, keşfetmek ya da kullanmak ister misin?

Java'dan hoşlandığın halde script yazmak için python, groovy, kotlin ve benzer dilleri mi kullanıyorsun?

`.java` dosyalarını doğrudan shell'de ya da konsolda kullanmak için Java 10+ desteğini kullanmayı biraz zor mu buldun?

O zaman bunu kolaylıkla yapabileceğin `jbang`'i denemelisin:
...
Anlık cli uygulaması, Java kullanılarak oluşturuldu. Ayrıca derleme ve çalıştırmada gerekli olduğu için picocli de dependency olarak eklendi.
...
...
ðŸº  /usr/local/Cellar/jbang/0.8.1: 18 files, 2.9MB, built in 6 seconds
```
+
. Verify the version.
+
